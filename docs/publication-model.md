...
Design direction and explicit pilot migration authorization recorded September 13, 2026. The maintainer approves a backward-incompatible replacement for the directory/identity model, reports that existing users have been informed, and owns communications. **Implementation cutover (September 19, 2026):** Draft-07 identity schemas, pilot YAML under `content/books/...`, mobile v2 setup/discovery, generated `api/v2/`, acceptance/smoke checks, and documented [v1 retirement](v1-retirement.md) are on `main` (`pre-v2-cutover`). v1 JSON continues to be generated for installed clients and rollback; removing it waits for the store release. Checklist: [v2-cutover-checklist.md](v2-cutover-checklist.md).
