...
Design direction and explicit pilot migration authorization recorded September 13, 2026. The maintainer approves a backward-incompatible replacement for the directory/identity model, reports that existing users have been informed, and owns communications. **Partial local implementation (September 18, 2026):** Draft-07 identity schemas, pilot fixtures under `fixtures/publication/`, identity validation, and generated `api/v2/` from those fixtures exist; the mobile client and production chapter layout still require the legacy contract. Do not move published sources under `content/books/...` or retire v1 until tooling/client acceptance checks pass together. The [roadmap checkpoint](roadmap.md#identity-migration-checkpoint) records the estimate, scope and exact resume sequence.
...
Implemented now: the bounded eight-chapter mapping supplies both Explore browse contexts, including partial selections and cached catalogs, without duplicate YAML, catalog rows or new API fields. Identity metadata schemas, `fixtures/publication/` for the pilot book/edition/adoptions, validation of those records, and generated `api/v2/` payloads from fixtures (held chapters excluded) are also in the tree. The production `content/` move, mobile book/edition selection, and the pilot setup flow remain authorized but not implemented. ISBN search is recorded for later implementation, not claimed as working.
