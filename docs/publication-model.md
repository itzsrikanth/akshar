...
Design direction and explicit pilot migration authorization recorded September 13, 2026. The maintainer approves a backward-incompatible replacement for the directory/identity model, reports that existing users have been informed, and owns communications. The design is authorized, not implemented: the validator, compiler, and mobile client still require the legacy contract. Implement and test the new tooling/client together before moving published sources or cutting over endpoints. The [roadmap checkpoint](roadmap.md#identity-migration-checkpoint) records the estimate, scope and exact resume sequence.
...
Implemented now: the bounded eight-chapter mapping supplies both Explore browse contexts, including partial selections and cached catalogs, without duplicate YAML, catalog rows or new API fields. Larger identity metadata schemas, the `content/` move, `api/v2/`, book/edition selection and the pilot setup flow are authorized but not implemented. ISBN search is recorded for later implementation, not claimed as working.
