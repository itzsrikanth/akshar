...
- [x] Follow-up: expose both Kannada/Grade3/Kannada and English/Grade5/Kannada in Explore using the exact existing mapping. Build choices from paired contexts, including partial prefixes, instead of only normalized canonical fields. Preserve the selected Grade 5 scope; both routes return the same eight chapter objects and fetch paths. This is not the general adoption schema or a search implementation.

## Identity and compatibility contract

The follow-up fixes a discovery gap: previously, complete saved Grade 5 scopes matched but Explore's automatic selection could discard the English prefix because choices came only from normalized Grade 3 metadata. `discoveryScopesFor` now supplies both existing scopes only for the eight explicitly mapped chapters, and hierarchy filtering/options use the same contexts. It does not invent English/Grade3 or Kannada/Grade5 combinations. Global catalog normalization still deduplicates to eight chapters; YAML, URLs, history identities, download keys and API output do not change. The English-medium Grade 5 usage is maintainer-reported; printed publication metadata remains Grade 3 Kannada first language.

Focused local checks cover both complete routes and every selection prefix, medium/grade choices, saved-scope auto-resolution, canonical-only/legacy-only cached catalogs, no crossed grade/medium pairs, unrelated books, no catalog mutation, and identical chapter references/download paths in both routes. New app code must reach users via the normal app deployment; a Git push alone does not update their installed JavaScript bundle. Simulator visual acceptance remains separate.
