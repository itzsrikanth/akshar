...
**Follow-up organization audit:** conversion is complete as YAML; a subsequent CI repair generated the missing v1 API artifacts, but no app/API identity migration is implemented. Splitting one book's chapters between Grade5/English and Grade3/Kannada directories is an interim storage inconsistency, not two publications or the final structure. The maintainer confirms both usages: Grade 3 first-language Kannada and Grade 5 second-language Kannada. The [publication/edition design](publication-model.md) records the exact official PDF match, corrected publisher identity, shared-edition adoption references, and the compatibility bridge required to reunify all eight chapters without changing learners' grades. The original conversion-only protection above does not authorize an untested later migration; follow `AGENTS.md` for that work.
