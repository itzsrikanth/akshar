...
**One-time pilot migration authorization, September 13, 2026:** the maintainer explicitly approves a backward-incompatible publication/edition/adoption migration and reports that existing users have been informed; the maintainer owns communications. This overrides the legacy-preservation requirements below for this transition only. A perpetual v1 projection, old-client support after the documented cutover, and automatic migration of all old history/downloads are not required. Implement a separate `api/v2/` contract and an explained, retryable new-client setup/reselection/redownload flow; do not serve v2 payloads at v1 URLs. Retiring v1 is authorized at the tested cutover, not while the current app still depends on it. Preserve a rollback snapshot. Use new local-state namespaces, leave old files/state untouched until successful setup and explicit cleanup, and do not silently clear unrelated preferences. Record implementation and deployment separately. This approval covers identity metadata schemas and layout/API/client changes, not invented content, new segment types, weakened validation, or release of held Grade 4 drafts. Future changes after this reset must again respect the supported contract. Resume from the [migration checkpoint](docs/roadmap.md#identity-migration-checkpoint); do not ask again for approval of this already agreed direction.
...
The bounded Kannada catalog correction is implemented in `docs/catalog-unification.md` and `catalog-compatibility.json`. Until the authorized v2 cutover, keep its canonical YAML under Kannada/Grade3, generate both v1 compatibility views, and normalize catalog views in clients rather than duplicating YAML. This bridge does not implement the publication schema or authorize arbitrary grade/medium remapping. Preserve its exact path mappings as migration/rollback evidence; replace hardcoded discovery aliases with paired adoption records during the approved migration.
...
**Quota-efficient order (mandatory for agent conversion):** (1) stabilize Kannada `source.*.yaml` and segment IDs; (2) generate latin/devanagari with local scripts (`indic_transliteration` / sanscript), reviewing exceptions only — do not burn chat-model quota on bulk transliteration; (3) use the LLM only for en/hi judgment calls, omitting unknowns; (4) convert **one grade or chapter at a time**, then validate/combine once — avoid parallel whole-book agents racing on holds/edition metadata. Savi Kannada FL expansion stops at **printed Grade 8** unless the maintainer asks otherwise.
