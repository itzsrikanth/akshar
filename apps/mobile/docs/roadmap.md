...
- [ ] Review the lean v2 pilot upgrade: keep existing v1 URLs and offer an explicit, explained reselection/redownload flow rather than building a full automatic history migration initially. Do not silently erase local data; obtain approval for that upgrade policy before implementation. See the [publication migration recommendation](../../../docs/publication-model.md#lean-initial-implementation-and-pilot-migration-recommendation).
...
- [ ] Wire sentence speaker availability to verified local audio or a known reachable remote asset; cached audio remains playable offline. Handle per-asset failures and recovery, not only boot status; see [media delivery](../../../docs/media-delivery.md).
...
- [ ] Source-word selection popup with reviewed root/gloss lookup and multi-word per-root display, without AI initially; prioritize other-grade Kannada demo content first. See [word selection](#6-word-selection-and-root-word-glosses).
...
- Allow selection of one or more words in the source-language text. Anchor a small floating popup above/below the selection without covering it; provide an accessible fallback sheet when space or platform selection APIs require it. Test Kannada grapheme clusters, wrapped lines, font scaling, and selection across punctuation; do not split vowel marks or assume ASCII word boundaries.
- Show the selected form, reviewed root/lemma if known, and its available English/Hindi or selected-language gloss. Reuse textbook vocabulary definitions when they actually match; do not infer a root by stripping a suffix or guess a definition from the sentence translation. Show “Root/meaning not available” for unmatched forms and label uncertain/ambiguous mappings rather than presenting one as certain.
- With multiple words selected, display each matched word/root and its individual meaning. Label this as individual word meanings, not a combined phrase translation. Sentence translations need not align one-to-one with Kannada words.
